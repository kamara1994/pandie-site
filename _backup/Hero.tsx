...
export default function Hero() {
  ...
  const slides = ["/hero04.png", "/hero02.jpg", "/hero03.png"];
  ...
  return (
    <section className="relative w-full overflow-hidden bg-[#0a1a10]" style={{ minHeight: "100svh" }}>
      {slides.map((src, i) => (
        <div key={src} className="absolute inset-0 duration-1500 transition-opacity" style={{ opacity: slide === i ? 1 : 0 }}>
          <Image src={src} alt="Children supported by Pandie Foundation" fill priority={i === 0} className="object-cover object-center" />
        </div>
      ))}
      ...
      <div className="relative z-10 flex min-h-[100svh] flex-col justify-center px-6 sm:px-12 lg:px-20 xl:px-28">
        <div className="max-w-3xl">
          <div className="flex items-center gap-3 transition-all duration-700" style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(20px)", transitionDelay: "100ms" }}>
          ...
          <div className="mt-8 h-[3px] rounded-full bg-[#c9962a] transition-all duration-1000" style={{ width: visible ? "80px" : "0px", transitionDelay: "500ms" }} />
          ...
          <div className="mt-10 flex flex-wrap gap-4 transition-all duration-700" style={{ opacity: visible ? 1 : 0, transitionDelay: "550ms" }}>
            <Link href="/donate" className="inline-flex items-center gap-3 bg-[#c9962a] px-8 py-4 text-[11px] font-bold uppercase tracking-[0.2em] text-[#0a1a10] transition-all duration-300 hover:bg-[#e8b84b] hover:-translate-y-0.5 hover:shadow-[0_8px_32px_rgba(201,150,42,0.4)]">
              Make an Impact <span className="text-base leading-none">→</span>
            </Link>
            <Link href="/about" className="inline-flex items-center gap-3 border border-white/30 px-8 py-4 text-[11px] font-bold uppercase tracking-[0.2em] text-white/80 backdrop-blur-sm transition-all duration-300 hover:border-white/60 hover:text-white">
              Our Story
            </Link>
          </div>

          <div className="mt-16 flex flex-wrap gap-10 border-t border-white/10 pt-10 transition-all duration-700" style={{ opacity: visible ? 1 : 0, transitionDelay: "700ms" }}>
            {[
