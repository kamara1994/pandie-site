    ...
    <main className="min-h-screen bg-[#f4f1ea]">
      ...
      <section className="relative overflow-hidden bg-[#0a1a10] px-6 py-24 text-white lg:px-20">
        ...
        <div className="relative mx-auto max-w-4xl">
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-[11px] font-semibold uppercase tracking-[0.32em] text-[#c9962a]">We'd Love to Hear From You</span>
          </div>
          <h1 className="font-heading text-[clamp(44px,6vw,80px)] font-semibold leading-[1.05] text-white">
            Get In<br /><em className="italic text-[#e8b84b]">Touch</em>
          </h1>
          <p className="mt-8 max-w-xl text-lg leading-8 text-white/65">Whether you want to donate, partner, volunteer, or simply learn more — we are here and we would love to connect.</p>
        </div>
      </section>
      ...
      <section className="px-6 py-20 lg:px-20">
        <div className="mx-auto max-w-6xl grid gap-12 lg:grid-cols-[1fr_1.4fr]">
          ...
          {/* Left: info */}
          <div>
            <span className="text-[11px] font-semibold uppercase tracking-[0.32em] text-[#c9962a]">Contact Information</span>
            <h2 className="mt-4 font-heading text-3xl font-semibold text-[#1a2e1f]">We answer every message personally.</h2>
            <p className="mt-4 text-[16px] leading-8 text-[#626a67]">Our team reviews every inquiry. Expect a response within 1–2 business days.</p>

            <div className="mt-10 space-y-6">
              {[
                { icon: "✉️", label: "Email", value: "info@pandiefoundation.org", href: "mailto:info@pandiefoundation.org" },
                { icon: "📞", label: "Phone", value: "+1 (307) 257-0001", href: "tel:+13072570001" },
                { icon: "📍", label: "USA Office", value: "United States of America", href: null },
                { icon: "🌍", label: "Operations", value: "Freetown, Sierra Leone", href: null },
                { icon: "🕐", label: "Hours", value: "Mon–Fri · 9:00 AM – 5:00 PM EST", href: null },
              ].map(({ icon, label, value, href }) => (
                <div key={label} className="flex items-start gap-5">
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center bg-[#214c34] text-xl">{icon}</div>
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#c9962a]">{label}</p>
                    {href ? <a href={href} className="mt-1 text-[15px] font-semibold text-[#1a2e1f] transition hover:text-[#c9962a]">{value}</a> : <p className="mt-1 text-[15px] font-semibold text-[#1a2e1f]">{value}</p>}
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-12 border-t border-[#1a2e1f]/10 pt-10">
              <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-[#c9962a] mb-5">Follow Our Mission</p>
          ...
          <div className="bg-white p-8 shadow-[0_2px_30px_rgba(0,0,0,0.07)] sm:p-10">
            {success ? (
              ...
              <>
                <h2 className="font-heading text-2xl font-semibold text-[#1a2e1f] mb-8">Send Us a Message</h2>
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="grid gap-5 sm:grid-cols-2">
                    {[
                      { field: "fullName" as const, label: "Full Name *", type: "text", placeholder: "Your full name" },
                      { field: "email" as const, label: "Email Address *", type: "email", placeholder: "you@example.com" },
                    ].map(({ field, label, type, placeholder }) => (
                      <div key={field}>
                        <label className="mb-2 block text-[11px] font-bold uppercase tracking-[0.18em] text-[#626a67]">{label}</label>
                  ...
                  <div>
                    <label className="mb-2 block text-[11px] font-bold uppercase tracking-[0.18em] text-[#626a67]">Phone (Optional)</label>
                    <input type="tel" placeholder="+1 (000) 000-0000" value={formData.phone}
                      onChange={e => setFormData(p => ({ ...p, phone: e.target.value }))}
                      className="w-full border border-[#e0dbd0] bg-[#faf8f5] px-4 py-3.5 text-[15px] text-[#1a2e1f] outline-none transition focus:border-[#c9962a] focus:bg-white placeholder:text-[#b0a898]" />
                  </div>
                  <div>
                    <label className="mb-2 block text-[11px] font-bold uppercase tracking-[0.18em] text-[#626a67]">Subject *</label>
                  ...
                  <div>
                    <label className="mb-2 block text-[11px] font-bold uppercase tracking-[0.18em] text-[#626a67]">Message *</label>
                    <textarea rows={5} placeholder="Tell us how you'd like to help or what you'd like to know..." value={formData.message}
