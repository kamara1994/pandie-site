  ...
  return (
    <main className="min-h-screen bg-[#f4f1ea]">
      ...
      <section className="relative overflow-hidden bg-[#0a1a10] px-6 py-24 text-white lg:px-20">
        ...
        <div className="relative mx-auto max-w-4xl">
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-[11px] font-semibold uppercase tracking-[0.32em] text-[#c9962a]">On The Ground · Sierra Leone</span>
          </div>
          <h1 className="font-heading text-[clamp(44px,6vw,80px)] font-semibold leading-[1.05] text-white">
            Events &<br /><em className="italic text-[#e8b84b]">Campaigns</em>
          </h1>
          <p className="mt-8 max-w-2xl text-lg leading-8 text-white/65">Every event is a direct act of care — bringing food, education, medical help, and hope to children who need it most.</p>
        </div>
      </section>
              ...
              <div className="relative min-h-[420px]">
                ...
                <div className="absolute bottom-8 left-8 right-8">
                  <span className="inline-flex items-center gap-2 bg-[#c9962a] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.2em] text-[#0a1a10] mb-4">
                    <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-[#0a1a10]" />Upcoming
                  </span>
                  <h2 className="font-heading text-3xl font-semibold text-white">{events[0].title}</h2>
                </div>
              </div>
                ...
                <div className="space-y-5 mb-8">
                  {[["📅", events[0].date], ["🕐", events[0].time], ["📍", events[0].location]].map(([icon, val]) => (
                    <div key={val} className="flex items-center gap-4">
                      ...
                      <span className="text-[15px] font-semibold text-white">{val}</span>
                    </div>
                  ))}
                </div>
                <div className="border-t border-white/10 pt-8">
                  <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#c9962a] mb-4">What to Expect</p>
                  <ul className="space-y-3">
                    {events[0].highlights.map(h => (
                      <li key={h} className="flex items-center gap-3 text-[14px] text-white/70">
                        <span className="h-1.5 w-1.5 rounded-full bg-[#c9962a] shrink-0" />{h}
                      </li>
                    ))}
                  </ul>
                </div>
        ...
        <div className="mx-auto max-w-6xl">
          <Reveal>
            <div className="flex items-center gap-4 mb-12">
              <div className="h-px w-10 bg-[#c9962a]" />
              <span className="text-[11px] font-semibold uppercase tracking-[0.32em] text-[#c9962a]">All Upcoming Events</span>
            </div>
          </Reveal>
              ...
              <Reveal key={event.title} delay={i * 80}>
                <article className="group overflow-hidden bg-white shadow-[0_2px_20px_rgba(0,0,0,0.06)] transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_16px_48px_rgba(0,0,0,0.1)]">
                  ...
                  <div className="p-7">
                    ...
                    <h3 className="font-heading text-xl font-semibold text-[#1a2e1f] leading-tight">{event.title}</h3>
                    <div className="mt-4 space-y-2">
                      <p className="text-[13px] text-[#626a67]">📅 {event.date} · {event.time}</p>
                      <p className="text-[13px] text-[#626a67]">📍 {event.location}</p>
                    </div>
                    <Link href="/donate" className="mt-6 inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.2em] text-[#214c34] transition hover:gap-4">
                      Support Event <span className="text-[#c9962a]">→</span>
                    </Link>
                  </div>
                </article>
              </Reveal>
      ...
      <section className="bg-[#0a1a10] px-6 py-20 text-center lg:px-20">
        <Reveal>
          <span className="text-[11px] font-semibold uppercase tracking-[0.32em] text-[#c9962a]">Get Involved</span>
