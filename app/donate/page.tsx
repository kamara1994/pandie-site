...
export default function DonatePage() {
  ...
  const successBox = (msg: string) => (
    <section className="bg-cream px-6 py-20 lg:px-20">
      <div className="mx-auto max-w-xl rounded-2xl border border-gold-500/25 bg-white p-10 text-center shadow-card">
        <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-forest-500 text-2xl text-white">✓</div>
        <h2 className="mt-6 font-heading text-3xl font-semibold text-forest-500">Thank you!</h2>
        <p className="mt-4 text-[15px] leading-7 text-muted">{msg}</p>
        <button
          onClick={() => { setSent(null); setActiveTab("money"); }}
          className="mt-8 rounded-xl bg-gold-500 px-7 py-3 text-[13px] font-bold uppercase tracking-[0.14em] text-forest-900 transition hover:bg-gold-400"
        >
          Back to giving
        </button>
      </div>
